...
module.exports = {
  ...
  updateEthereumAddress: async (req, res) => {
    const { id } = req.params;
    const { ethereumAddress } = req.body;

    try {
        const updateData = { ethereumAddress: ethereumAddress };
        const internaute = await internauteModel.findByIdAndUpdate(
            id,
            updateData,
        ...
        res.status(200).json({
            success: true,
            message: "Internaute Ethereum address updated successfully",
            data: internaute
        });
    } catch (error) {
        console.error("Error updating Ethereum address internaute:", error);
        res.status(500).json({
            success: false,
            message: "Error updating Ethereum address internaute: " + error.message,
            data: null
        });
    }
  },
  updateEthereumPrivateKey: async (req, res) => {
    const { id } = req.params;
    const { ethereumPrivateKey } = req.body;

    try {
        const updateData = { ethereumPrivateKey: ethereumPrivateKey };
        const internaute = await internauteModel.findByIdAndUpdate(
            id,
            updateData,
            { new: true } // Option pour retourner le document modifié
        ...
        res.status(200).json({
            success: true,
            message: "Internaute ethereumPrivateKey updated successfully",
            data: internaute
        });
    } catch (error) {
        console.error("Error updating ethereumPrivateKey internaute:", error);
        res.status(500).json({
            success: false,
            message: "Error updating ethereumPrivateKey internaute: " + error.message,
            data: null
        });
    }
  },
